...
/// <reference path="~/Scripts/Underscore/underscore.js" />
...
/// <reference path="~/Scripts/Panopto/RenderBranding.js" />
...
Panopto.Application.prototype =
{
    ...
    init: function ()
    {
        ...
        Panopto.Core.UI.Components.popup($("#loginDropdown"), $("#loginLink, #loginButton"), null, null, true);
        Panopto.Core.UI.Components.popup($("#supportDropdown"), $("#supportLink"), null, null, true, true);
        ...
        // Wire up the mobile launch button so that it is context aware
        Panopto.Core.UI.Handlers.button($("#launchIosApp"), function () {
            // Get the recorder launch url
            var url = Panopto.Application.defaultInstance.getRecorderLaunchUrl(
                Panopto.Core.Constants.AppBrowseAction);
    ...
    showNotificationBanner: function (message, messageId, dismissable, iconClass) {
        ...
        $notificationBanner.find("#notificationMessageText").text(message);
                ...
                if (redirect)
                {
                    if (!window.location.href.startsWith(Panopto.loginUrl))
                    {
                        ...
                        window.location.href = String.format(
                            "{0}?AuthExpired=true&ReturnUrl={1}",
                            Panopto.loginUrl,
                            url);
                    }
                }
    ...
    initCreateMenu: function ()
    {
        ...
        if ((Panopto.user.isVideographer || Panopto.user.isCreator || Panopto.user.hasWriteAccess)
            && !Panopto.isHostedMaster)
        {
            // Show the create menu
            $("#createMenu").show();

            Panopto.Core.UI.Components.popup($("#createDropdown"), $("#createButton"), null, null, true, true);
             
            $(".menuItem").keydown( function(e) 
            {
                if(e.keyCode === Panopto.Core.Key.Esc){
                    $("#createDropdown").hide();
                    $("#createButton").focus();
                }
                // Prevent presses from propagating up to BODY and re-hiding the drop-down, especially for tabbing
                e.stopPropagation();
            });

            // Style the header login based on whether the user is logged in.
            ...
            var showCreateSession = function (createParams, headerText)
            {
                ...
                if (createParams.schedule) {
                    Panopto.Application.defaultInstance.updateState({
                        ...
                        modalHeader: headerText, // all values are resource strings, so this is safe
...
Panopto.Application.SetClassName = function (className) {
    ...
    var $content = $("#content"),
        classList = ["content-allSessions", "content-scheduled", "content-processing",
                     "content-allFolders", "content-usage", "content-users", "content-groups",
                     "content-remoteRecorders", "content-licenses", "content-remote-recorders"];
