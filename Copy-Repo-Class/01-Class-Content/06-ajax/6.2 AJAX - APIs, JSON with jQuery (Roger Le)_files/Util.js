...
Panopto.Util.displayDeliveryName = function (deliveryItem)
{
    ...
    return Panopto.Core.TextHelpers.cleanTextWithHighlighting(
        Panopto.Util.getDeliveryName(deliveryItem)
    );
};
...
Panopto.Util.displayDeliveryNameWithoutHighlighting = function (deliveryItem)
{
    ...
    return Panopto.Core.TextHelpers.cleanTextWithoutHighlighting(
        Panopto.Util.getDeliveryName(deliveryItem)
    );
};
...
Panopto.Util.getDateMessage = function (sessionRow) {
    ...
    switch (sessionRow.Status) {
        ...
        case Panopto.Data.SessionStatus.Scheduled:
            return Panopto.GlobalResources.Sessions_List_ScheduledFor.format(
                        Panopto.Util.displayDate(sessionRow.StartTime, { format: "friendly" }));
...
{
    var url = item.ViewerUrl;

    // Not currently a user-exposed feature.
    if (false) 
    {
        // Swapping this for the line below allows viewer links from the session list to always start wherever the viewer last left off.
        if (time) 
        {
            url += "&start=" + time;
        }
        else if (item.MostRecentViewTime &&
                 item.Duration && 
                 ((item.Duration - item.MostRecentViewTime) > 30) &&
                 item.MostRecentViewTime > 30) 
        {
            // Use the most recent time if it's specified, is greater than 30 seconds into the recording 
            // and is less than 30 seconds from the end of the recording
            url += "&start=" + item.MostRecentViewTime;
        }
    }
    else 
    {
...
Panopto.Util.handleThumbnailLoad = function ($container)
{
    ...
    $container.find("a:visible .thumbnail-img, span:visible .thumbnail-img")
        ...
        // IE8 (and 9?) may have already fired the "load" event for cached images
        .each(function ()
        {
